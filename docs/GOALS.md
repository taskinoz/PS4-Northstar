...
1. **Identify what `r15` is at the `menu_ns_modmenu.nut` crash site.** Profiling `internal_vm+0x40a0` on our own known UI VM (2026-08-07) showed that table is healthy and populated during a normal boot — so "the table is unpopulated" is ruled out. The likely explanation now is that the crashing function's `r15` is a *different* object than the UI VM we've been probing (the `+0x50` offset match was coincidental, not proven identity), most plausibly a compile-time type/class resolution context specific to parsing a typed struct parameter. This needs either a debugger attached to shadPS4 at the fault, or static backward-tracing from the crash site through its caller(s) — see TECHNICAL-NOTES.md for the full writeup before continuing.
...
3. ~~Re-verify `ns_allow_team_changes` a second time~~ **Done (2026-08-07):** second clean run, no address-space issue this time (confirms that was transient host contention, not a code fault). **Still open**: confirm the registered ConVar actually changes gameplay behavior in a live match, not just that `FindVar` returns the right object identity — that requires a real Northstar server connection and a team-based gamemode, not attempted yet.
...
- Compiling a real mod UI script with a typed struct parameter (`ui/menu_ns_modmenu.nut`, using `ModInfo`) via `CompileList` crashes at a real (non-null) address reading `internal_vm+0x40a0`. That table was profiled and is healthy on our own UI VM during a normal boot, so the crashing code is most likely operating on a *different* object than our probed UI VM, not an unpopulated table — root cause still open (needs a debugger or static backward-tracing). Not all mod scripts are necessarily affected — only one has been tested — but treat any `-EnableM6ScriptInjectFromMods` script as unverified. See TECHNICAL-NOTES.md.
