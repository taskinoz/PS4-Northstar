...
1. **Test a real insecure server connection end-to-end.** Set `ns_auth_allow_insecure 1` on a real Northstar server (PC-hosted is fine) and attempt Goal 0's direct-connect flow from the PS4/shadPS4 client. This is now the practical path to Goal 8 (see its re-scoped description) and, if it works, unblocks live verification of `ns_allow_team_changes`'s gameplay effect and gives a real environment to test Goal 6 script loading in.
