...
1. **Test a real insecure server connection end-to-end.** A local dedicated server (`+ns_auth_allow_insecure 1`) was stood up and confirmed listening (2026-08-07), but triggering the client's `connect` command turned out to need more than expected: **loose `.cfg` files are not executed at boot on this retail PS4 build at all** (verified — the kernel FS log shows no `.cfg` file opened during boot except save-data `profile.cfg`; tried both Northstar's own `autoexec_ns_client.cfg` via the r2 overlay and the vanilla `autoexec_client.cfg` via the proven Stage 1 VPK pipeline, neither fired). See TECHNICAL-NOTES.md for the full writeup. Two ways forward, neither started: (a) real GUI/input automation against the shadPS4 window (outside current tooling) to drive the existing in-game direct-connect menu by hand, or (b) locate and call the engine's console-command execution path (`Cbuf_AddText`/`Cbuf_Execute` or equivalent) directly from the native PRX, the same way `CompileList` was found and safely called — new reverse-engineering work, not started. (a) is the faster near-term unblock if a human is available to type an IP into the menu once shadPS4 is running.
